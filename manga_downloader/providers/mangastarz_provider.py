"""Manga Starz provider — site-specific discovery and download logic.

Shared infrastructure (browser, Cloudflare, session, download engine,
export engine) is inherited from BaseMangaProvider.
"""
...
class MangaStarzProvider(BaseMangaProvider):
    """Manga Starz (manga-starz.net) provider."""

    SITE_NAME = "Manga Starz"
    SITE_DOMAINS = ["manga-starz.net"]

    async def discover_manga(self, url: str) -> DiscoveryResult:
        """Discover manga info and chapters from a Manga Starz manga page."""
    ...
    async def discover_chapter_page(
        ...
        Dedicated flow for direct chapter URLs. Navigates to chapter page,
        extracts images directly without chapter list discovery.
        """
        if self.scraper is None:
            raise RuntimeError("Provider not initialized.")
